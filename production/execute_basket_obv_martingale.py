...
def enter_group_trades(select_currency, pair, curr_price, avg_prices, account_numbers, is_buy, max_equity, order_amount, open_delta, revert_count):
	...
	if order_amount is None:
		new_order.base_amount = (max_equity / 5000) * 4000 
	else:
...
def open_position(account_numbers, max_equity, select_currency, delta_map, avg_prices, pair_bid_ask_map, threshold_delta, is_revert_entry):
...
def monitor_accounts():
	...
	cursor = cnx.cursor()
	query = ("""
		SELECT account_nbr, api_key, select_currency, max_equity, is_demo, last_updated, threshold_delta, is_revert_entry
		from managed_accounts 
		where model_type = 'obv_martingale'
		""")
	...
	for row1 in setup_rows:
		...
		if time_diff >= 22 and total_float_pip_profit >= 400:
		...
		if total_positions == 0:
			...
			if margin_used < margin_available:
				...
				is_success = open_position([account_number], max_equity, select_currency, delta_map, avg_prices, pair_bid_ask_map, threshold_delta, is_revert_entry)
			else:
