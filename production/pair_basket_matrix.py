...
def replay_data(base_currency, X_train, sl_pips, tp_pips):
    ...
    for index in range(len(X_train)):
        ...
        for pair in currency_pairs:
            if pair not in price_pair_map:
                price_pair_map[pair] = []

            price_pair_map[pair].append(open_price_map[pair])
            if len(price_pair_map[pair]) > 2:
                price_pair_map[pair] = price_pair_map[pair][1:]
        ...
        for item in sorted_pairs:
            ...
            z_score = (price_pair_map[item[0]][-1] - np.mean(price_pair_map[item[0]])) / np.std(price_pair_map[item[0]])
            ...
            if abs(item[1]) > 1:
                ...
                same_pair_order_count = len([o for o in orders if o.pair == order.pair])
        ...
        for order in orders:
            ...
            if min(profit_high, profit_low) < -50:
                ...
                new_order.open_delta = pair_delta[order.pair]
            ...
            if order.pair not in total_pips_by_pair_map:
                total_pips_by_pair_map[order.pair] = 0

            total_pips_by_pair_map[order.pair] += profit
        ...
        if total_amount >= 5:
            min_order = None
            for order in orders:
                if min_order is None or order.profit > min_order.profit:
                    min_order = order

            print ("close excess", min_order.profit, "-------------")
            equity += min_order.profit
            float_profit -= min_order.profit
            prev_size = len(orders)
            orders = [order for order in orders if order != min_order]
            if len(orders) != (prev_size - 1):
                print ("wrong size", prev_size, len(orders))
                sys.exit(0)
        '''
