...
def close_group(group_metadata, balance, float_profit, margin_used, margin_available, orders):
	...
	if (time.time() - last_time > 12 * 60 * 60) or (margin_used / margin_available > 1.1):
		...
		if balance / equity_mean > 1.1 or float_profit / margin_used > 1.1 or (margin_used / margin_available > 1.1):
			...
			os.remove(root_dir + "group_metadata_news")
			os.remove(root_dir + "order_metadata_news")
			os.remove(root_dir + "model_wt")
			return

		group_metadata['equity_curve'].append(balance)
		group_metadata['group_processed_time'] = time.time()
		pickle.dump(group_metadata, open(root_dir + "group_metadata_news", 'wb'))

def close_group_trades(orders_by_model, model_key, prediction, order_metadata, group_metadata, total_margin_available, total_margin_used, existing_order_amount, curr_price, avg_prices, avg_spreads, total_models):
	...
	for order in orders:
		...
		if order.dir != prediction and group_metadata[order.pair + "_curr_spread"] < 5.0: 
...
def enter_group_trades(model_key, pair, prediction, order_metadata, group_metadata, orders_by_model, curr_model_wt, prev_model_wt, free_margin, used_margin, curr_price, account_numbers, existing_order_amount, curr_spread, avg_prices):
	...
	if base_model_key in orders_by_model:
		...
		if (prediction < 0) == (sell_count >= buy_count):
			group_metadata[base_model_key + "_reduce_amount"] = 0
	...
	if  abs(prediction) >= group_metadata[base_model_key + "_reduce_amount"]:
		...
		group_metadata[base_model_key + "_reduce_amount"] = abs(prediction) + 0.1
...
def process_pending_trades(account_numbers, avg_spreads):
	...
	if os.path.isfile(root_dir + "group_metadata_news"):
		group_metadata = pickle.load(open(root_dir + "group_metadata_news", 'rb'))
	else:
		group_metadata = {}

	if os.path.isfile(root_dir + "order_metadata_news"):
		order_metadata = pickle.load(open(root_dir + "order_metadata_news", 'rb'))
	else:
		order_metadata = {}

	if os.path.isfile(root_dir + "group_base_wt_news"):
		prev_model_wt = pickle.load(open(root_dir + "group_base_wt_news", 'rb'))
	else:
		prev_model_wt = {}
	...
	base_calendar = pd.DataFrame.from_records(download_calendar.download_calendar(604800), columns=['currency', 'impact', 'actual', 'forecast', 'time', 'region'])
	...
	for pair in currency_pairs: 
		...
		if pair + "_last_calendar_update" not in group_metadata:
			group_metadata[pair + "_last_calendar_update"] = time.time()

	
		if (time.time() - group_metadata[pair + "_last_calendar_update"]) < 60 * 60 and False:
			continue

		group_metadata[pair + "_last_calendar_update"] = time.time()

		for currency in [pair[0:3], pair[4:7]]:

			model_key = pair + "_" + currency

			calendar = base_calendar[base_calendar["currency"] == currency]
			last_time = calendar["time"].values.tolist()[-1]

			if len(calendar[calendar["time"] >= time.time() - (36 * 60 * 60)]) == 0:
				close_group_trades(orders_by_model, model_key, None, \
			...
			if (time.time() - last_time) < 12 * 60 * 60 and model_key not in orders_by_model:
				continue

			if len(calendar[calendar["time"] >= time.time() - (24 * 60 * 60)]) == 0:
				continue

			prices, times = get_time_series(pair, 12)

			deltas = []
			for price in prices:
				deltas.append(price - prices[0])

			z_delta = (deltas[-1] - np.mean(deltas)) / np.std(deltas)
			curr_model_wt = float(total_balance) * 0.05
	
			base_model_key = enter_group_trades(model_key, pair, z_delta, order_metadata, \
			...
			close_group_trades(orders_by_model, model_key, z_delta < 0, \
			 	order_metadata, group_metadata, total_margin_available, total_margin_used, \
			  	existing_order_amount, curr_price, avg_prices, avg_spreads, \
			  	len(orders_by_model))


	pickle.dump(prev_model_wt, open(root_dir + "group_base_wt_news", 'wb'))
	pickle.dump(group_metadata, open(root_dir + "group_metadata_news", 'wb'))
	pickle.dump(order_metadata, open(root_dir + "order_metadata_news", 'wb'))
...
trade_logger = setup_logger('first_logger', root_dir + "trade_news.log")
...
if get_mac() == 154505288144005:
	...
	sftp.put("/tmp/pair_avg_spread", root_dir + "group_metadata_news")
	sftp.put("/tmp/pair_avg_price", root_dir + "order_metadata_news")
