...
[fpr1, tpr1] = pickle.load(open("/tmp/old_auc", 'rb'))
[fpr2, tpr2] = pickle.load(open("/tmp/new_auc", 'rb'))

# method I: plt
import matplotlib.pyplot as plt
plt.title('Receiver Operating Characteristic')
plt.plot(fpr1, tpr1, 'r', label = 'Existing Phone Mount Model AUC = %0.2f' % metrics.auc(fpr1, tpr1), color='r')
plt.plot(fpr2, tpr2, 'r', label = 'Markov Model Segmentation AUC = %0.2f' % metrics.auc(fpr2, tpr2), color='b')
...
'''precisions = pickle.load(open("/tmp/precisions", 'rb'))
recalls = pickle.load(open("/tmp/recalls", 'rb'))
thresholds = pickle.load(open("/tmp/thresholds", 'rb'))

ratios = [a / b for a, b in zip(precisions, recalls)]


# method I: plt
import matplotlib.pyplot as plt
plt.title('Precision vs Recall')
#plt.plot(thresholds, ratios, 'b', label = 'Precision / Recall', color='r')

plt.plot(recalls, precisions, 'b', label = 'Precision / Recall', color='r') 
plt.legend(loc = 'lower right')
...
for year in range(2008, 2019):
	...
	total_pnl = 0
	pairs = ["GBP_AUD", "USD_JPY", "EUR_JPY", "GBP_JPY", "NZD_JPY", "CAD_JPY", "CHF_JPY", "AUD_JPY"]
	...
	pairs = ["NZD_CAD"]#, "AUD_CHF", "NZD_CHF", "GBP_CHF", "CAD_CHF", "AUD_CAD", "AUD_NZD"]
	...
	#["EUR_JPY", "USD_JPY", "EUR_AUD", "EUR_CAD", "EUR_GBP", "AUD_CAD", "GBP_CAD", "NZD_CAD", "AUD_NZD"]:
		...
		for test_pair in [currency_pair]:
			...
			if test_pair[4:7] != "JPY":
				...
				total_pnl += equity_curve[-1] * 0.001
