...
def build_news_description_release_model(year, df, description, price_df, equity):
	...
	for model_index in range(40):
		train_indexes = range(len(y_train))
		rand.shuffle(train_indexes)
		train_indexes = train_indexes[:int(0.25 * len(train_indexes))]

		clf = GradientBoostingRegressor(random_state=42)
		clf.fit([X_train[i] for i in train_indexes], [y_train[i] for i in train_indexes])
	...
	for index in range(len(X_test)):
		...
		predictions = [model.predict([X_test[index]])[0] for model in model_set]
		#print predictions

		mean = np.mean(predictions)
		std = np.std(predictions)

		z_scores = [(p - mean) / std for p in predictions]

		orders = []
		for z_score, price in zip(z_scores, y_test[index]):

			order = Order()
			order.open_price = price
			order.dir = z_score < 0
			order.amount = abs(z_score)
			orders.append(order)

		for order in orders:

			if order.dir == (y_test[index][-1] > order.open_price):
				equity += abs(y_test[index][-1] - order.open_price) * order.amount
			else:
				equity -= abs(y_test[index][-1] - order.open_price) * order.amount
	...
	for description in description_count_map:
		if description_count_map[description] > 60:
			...
			for year in range(2007, 2018):
				...
				ec, equity = build_news_description_release_model(year, df, description, price_df, equity)
				equity_curve += ec
			...
			'''
			if sharpes[-1] > 0.2:
				ec, equity, model = build_news_description_release_model(None, df, description, price_df, equity)
