...
def back_test_news_calendar(curr_calendar_df, select_pair, price_df):
	...
	for index, row in curr_calendar_df.iterrows():
		...
		for index2, calendar_row in test_calendar.iterrows():
			...
			for barrier_index in range(1, 21):
				...
				for price in future_prices[new_hour_lag:]:
...
checkIfProcessRunning('analyze_historic_news_signals.py', select_pair)
...
query = ("SELECT max(time_stamp) FROM historic_news_barrier_probs where \
					currency_pair = '" + select_pair + "' and model_key='M4' \
					")
