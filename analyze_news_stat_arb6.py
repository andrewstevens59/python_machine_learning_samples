...
def find_price_delta(prices, end_index, start_index):
	price_diff = [prices[end_index][index] - prices[start_index][index] for index in range(len(prices[0]))]
	return price_diff[0] - np.mean(price_diff[1:])

def find_price_delta2(future_price, anchor_price):
	price_diff = [future_price[index] - anchor_price[index] for index in range(len(future_price))]
	return price_diff[0] - np.mean(price_diff[1:])

def calculate_pnl(future_price, order, future_mult_factor, commission):
	...
	for index in range(len(future_price)):
		...
		if index == 0:
			if (future_price[index] > order.open_price[index]) == (order.dir):
				total_pnl += (abs(future_price[index] - order.open_price[index]) - commission) * order.amount * future_mult_factor * 0.5
			else:
				total_pnl += (-abs(future_price[index] - order.open_price[index]) - commission) * order.amount * future_mult_factor * 0.5

		else:
			if (future_price[index] > order.open_price[index]) != (order.dir):
				total_pnl += (abs(future_price[index] - order.open_price[index]) - commission) * order.amount * future_mult_factor * 0.1 * (1.0 / (len(future_price) - 1))
			else:
				total_pnl += (-abs(future_price[index] - order.open_price[index]) - commission) * order.amount * future_mult_factor * 0.1 * (1.0 / (len(future_price) - 1))

	return total_pnl

def create_training_set(currency1, currency2, price_df, pair, pairs, year, test_calendar, description_map, feature_importances):
	...
	for index, row in test_calendar.iterrows():
		...
		future_prices = (price_df[price_df['times'] >= row['time']])[['prices'] + pairs].head(24 * 5 * 2).values.tolist()

		back_prices = (price_df[price_df['times'] <= row['time']])[['prices'] + pairs].tail(24).values.tolist()
		...
		for index in range(1, 12):
			deltas1.append(find_price_delta(future_prices, index, 0))

		deltas2 = []
		for index in range(0, len(back_prices) - 1):
			deltas2.append(find_price_delta(future_prices, -1, index))
		...
		for i in [2, 4, 8]:
			...
			global_feature_vector += description_vector1_curr1 + description_vector2_curr1 
		...
		y_train.append(find_price_delta(future_prices, -1, 0))

	return X_train, y_train



def back_test(currency1, currency2, price_df, pair, year, test_calendar, model_mean, description_map, feature_importances, interest_times):
	...
	for index, row in test_calendar.iterrows():
		...
		for future_price, future_time, future_mult_factor in zip(future_prices, future_times, future_mult_factors):
			...
			if future_time not in std_map:
				std = (price_df[price_df['times'] <= future_time])['prices'].tail(24 * 5 * 4).diff(periods=4).std()
				std_map[future_time] = std
			else:
				std = std_map[future_time]
			...
			'''
			found_interest_release = False
			for interest_time in interest_times:
				if abs(interest_time - future_time) < 60 * 60 * 24 * 2 and future_time > interest_time:
					found_interest_release = True
					break

			if found_interest_release:
				break
			'''
...
def create_data_set(year_range, currency_pair, pairs, select_year, description_map, feature_importances):
	...
	for year in year_range:
		...
		for test_pair in [currency_pair]:
			...
			x, y = create_training_set(first_currency, second_currency, price_df, test_pair, pairs, year, test_calendar, description_map, feature_importances)
...
for year in range(2009, 2019):
	...
	chosen_pairs = ["USD_JPY", "EUR_JPY", "GBP_JPY", "NZD_JPY", "CAD_JPY", "CHF_JPY", "AUD_JPY"]
	for currency_pair in chosen_pairs:
	...
	#["EUR_JPY", "USD_JPY", "EUR_AUD", "EUR_CAD", "EUR_GBP", "AUD_CAD", "GBP_CAD", "NZD_CAD", "AUD_NZD"]:
		...
		select_pairs = list(filter(lambda x: x != currency_pair, chosen_pairs))

		print select_pairs

		X_train, y_train, X_test, y_test = create_data_set(range(year - 1, year), currency_pair, select_pairs, None, description_map, [])
		...
		for test_pair in [currency_pair]:
			...
			price_df = align_prices(select_pairs, price_df)

			equity_curve, returns, avg_amount = back_test(first_currency, second_currency, price_df, test_pair, select_pairs, year, test_calendar, clf_mean, description_map, [])
