...
def min_volatility(df, optimization_bounds = (0.0, 1.0)):
...
def back_test(currency1, currency2, price_df, pair, pairs, year, test_calendar, model_mean, description_map, feature_importances, std_map):
	...
	for index, row in test_calendar.iterrows():
		...
		prediction = ir.predict([mean_prediction])[0]

		if math.isnan(prediction) == False:
		...
		for future_price, future_time, future_mult_factor in zip(future_prices, future_times, future_mult_factors):
			...
			if abs(future_price - anchor_price) > max_delta * 2:
				...
				if future_time not in std_map:
					std_map[future_time] = min_volatility((price_df[price_df['times'] <= future_time])[pairs].tail(24 * 5 * 4).diff(periods=24), optimization_bounds = (-1.0, 1.0))
				...
				new_order.amount = (abs(delta) * abs(std)) / (time_step * 0.1)
	...
	return equity_curve, returns, np.mean(order_amounts), std_map
...
for year in range(2012, 2019):
	...
	total_pnl = 0
	pairs = ["AUD_CAD", "USD_JPY", "EUR_JPY", "GBP_JPY", "NZD_JPY", "CAD_JPY", "CHF_JPY", "AUD_JPY"]
	#pairs = ["EUR_USD", "GBP_USD", "NZD_USD", "USD_CHF", "USD_CAD", "AUD_USD"]
	#pairs = ["NZD_CAD", "AUD_CAD", "AUD_NZD"]
	#pairs = ["USD_CAD", "AUD_USD"]
	#pairs = currency_pairs
	#pairs = ["GBP_AUD", "EUR_AUD", "EUR_CAD", "EUR_GBP", "AUD_CAD", "GBP_CAD", "NZD_CAD", "AUD_NZD"]
	...
	#["EUR_JPY", "USD_JPY", "EUR_AUD", "EUR_CAD", "EUR_GBP", "AUD_CAD", "GBP_CAD", "NZD_CAD", "AUD_NZD"]:
		...
		for test_pair in [currency_pair]:
			...
			equity_curve, returns, avg_amount, std_map = back_test(first_currency, second_currency, price_df, test_pair, pairs, year, test_calendar, clf_mean, description_map, feature_importances, std_map)
