	...
	for iteration in range(cross_val_num):
		...
		for index in range(len(instance_offsets)-1):
			if index in true_indexes + false_indexes:
				...
				X_train += X[instance_offsets[index]:instance_offsets[index+1]]
				y_train += y[instance_offsets[index]:instance_offsets[index+1]]
	...
	for index, row in curr_calendar_df.iterrows():
		...
		for index2, calendar_row in test_calendar.iterrows():
			...
			for time_offset in range(1, min(24, len(future_prices))):
				...
				X_last = [feature1, feature2, future_prices[time_offset] - future_prices[0], time_offset]
		...
		X_last = [feature1, feature2, prices[-1] - prices[0], hour_lag]
...
checkIfProcessRunning('analyze_historic_news_lags_signals1.py', select_pair)
...
query = ("SELECT max(time_stamp) FROM historic_news_barrier_probs where \
					currency_pair = '" + select_pair + "' and model_key = 'BT2' \
					")
