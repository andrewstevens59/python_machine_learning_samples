...
def back_test(select_pair, adjust_factor, is_hedge, auc_barrier_mult, 
    ...
    for barrier_index in predictions[2007]:
        forecasts = []
        for select_year in range(2007, 2009):
            forecasts += list(predictions[select_year][barrier_index]["predictions"])

        forecasts = [abs(v - 0.5) for v in forecasts]
    ...
    for select_year in range(2007, 2009):
        ...
        for barrier_index in predictions[select_year]:
            ...
            for time_index in range(len(predictions[select_year][barrier_index]["predictions"])):
               
                timestamp = predictions[select_year][barrier_index]["timestamp"][time_index]
                predict = predictions[select_year][barrier_index]["predictions"][time_index]

                if timestamp not in forecasts:
                    forecasts[timestamp] = []

                if abs(predict - 0.5) > barrier_percentiles[barrier_index]:
                    forecasts[timestamp].append((predict - 0.5) * barrier_index)
        ...
        for item in series:
            ...
            for between_price, between_time in zip(between_prices, between_times):
                ...
                for order in orders:
                    ...
                    if  (((pip_diff < -max_pip_barrier) or (pip_diff > max_pip_barrier * reward_risk_ratio))) or (len(orders) - order_count >= 10) or (inst_profit < -50 * growth_factor * adjust_factor) or (inst_profit > 50 * growth_factor * adjust_factor * reward_risk_ratio):
                        total_profit += profit
                        order_count += 1
            ...
            if abs(avg_count) > 0:# and (len(orders) == 0 or abs(exchange_rate - price_deltas[-1]) > pip_size * 10):
                ...
                if abs(signal) > 0 and std1 > min_trade_volatility and (signal > 0) != curr_trade_dir:
