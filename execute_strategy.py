...
def close_group_trades(pair, orders_by_model, model_key, order_metadata, group_metadata, total_margin_available, total_margin_used, existing_order_amount, curr_price, avg_prices, avg_spreads, total_models, white_base_model_key_set):
	...
	if (model_key + "_last_profit_udpate") not in group_metadata:
		...
		pickle.dump(group_metadata, open(root_dir + "group_metadata_test", 'wb'))
	...
	for order in orders:
		...
		pnl_factor = order.PnL / ((avg_prices[order.pair] - order.open_price) * order.portfolio_wt)
		...
		'''
		if (total_pnl > 0 and total_pnl > abs(group_metadata[model_key + "_min_profit"]) * 1.5) and group_metadata[order.prediction_key + "_curr_spread"] < 5.0: 
		...
		if (abs(order.curr_prediction - order.open_prediction) > 1.0) and total_pnl > 0 and group_metadata[order.prediction_key + "_curr_spread"] < 5.0: 
			order_metadata, existing_order_amount, is_success = close_order(order, order_metadata, existing_order_amount, curr_price, commission, pip_size)
			if is_success == True:
				continue
		

		if (abs(order.curr_prediction - order.open_prediction) > 0.5) and total_pnl < 0 and group_metadata[order.prediction_key + "_curr_spread"] < 5.0: 
			order_metadata, existing_order_amount, is_success = close_order(order, order_metadata, existing_order_amount, curr_price, commission, pip_size)
			if is_success == True:
				continue
...
def convert_portfolio_pairs(portfolio_wts, avg_prices):


	new_portfolio = {}
	for pair in portfolio_wts:

		first_currency = pair[0:3]
		second_currency = pair[4:7]

		if first_currency == "USD" or second_currency == "USD":
			new_portfolio[pair] = portfolio_wts[pair]
			continue

		new_pair = "USD_" + first_currency
		if new_pair not in currency_pairs:
			new_pair = first_currency + "_USD"
			wt = portfolio_wts[pair]
		else:
			wt = -portfolio_wts[pair]

		wt *= avg_prices[second_currency + "_USD"]
		new_portfolio[new_pair] = wt
...
def enter_group_trades(pair, white_model, order_metadata, group_metadata, orders_by_model, curr_model_wt, prev_model_wt, free_margin, used_margin, curr_price, account_numbers, existing_order_amount, curr_spread, avg_prices):
	...
	entry = (abs(prediction) >= group_metadata[base_model_key + "_reduce_amount"])
	
	if  entry and curr_spread < 1.2 and len(orders) == 0:

		portfolio_wts = convert_portfolio_pairs(white_model['wts'], avg_prices)
		...
		for pair in portfolio_wts:
			...
			'''
			new_order.base_amount = 1 + len(orders)

			if len(orders) > 1:
				new_order.base_amount = min(max(1, len(orders)), max(1, orders[-1].amount + 1))

			if martingale_type == "flip":
				new_order.base_amount = abs(buy_amount - sell_amount) + 1
			'''

			if base_model_key not in prev_model_wt:
				prev_model_wt[base_model_key] = curr_model_wt

			new_order.base_amount = curr_model_wt * abs(portfolio_wts[pair]) * abs(prediction)

			if all_JPY == True:
				order_amount = int(round((new_order.base_amount)))

			if order_amount == 0:
				continue

			if (prediction < 0) == (wt > 0):
				new_order.dir = True
			else:
				new_order.dir = False
			...
			if 'orderFillTransaction' in order_info:
				...
				new_order.portfolio_wt = portfolio_wts[pair]
...
def process_pending_trades(account_numbers, avg_spreads):
	...
	if os.path.isfile(root_dir + "group_metadata_test"):
		group_metadata = pickle.load(open(root_dir + "group_metadata_test", 'rb'))
	else:
		group_metadata = {}

	if os.path.isfile(root_dir + "order_metadata_test"):
		order_metadata = pickle.load(open(root_dir + "order_metadata_test", 'rb'))
	else:
		order_metadata = {}

	if os.path.isfile(root_dir + "group_base_wt_test"):
		prev_model_wt = pickle.load(open(root_dir + "group_base_wt_test", 'rb'))
	else:
		prev_model_wt = {}
	...
	for white_model in scalper_list:
		...
		prediction_key = white_model['prediction_key'] 
		martingale_type = white_model['martingale_type'] 
		wts = convert_portfolio_pairs(white_model['wts'], avg_prices)
	...
	for model in whitelist:
		...
		curr_model_wt = float(total_balance) * 1
	...
	pickle.dump(prev_model_wt, open(root_dir + "group_base_wt_test", 'wb'))
	pickle.dump(group_metadata, open(root_dir + "group_metadata_test", 'wb'))
	pickle.dump(order_metadata, open(root_dir + "order_metadata_test", 'wb'))
...
trade_logger = setup_logger('first_logger', root_dir + "trade_test.log")

#process_pending_trades(["101-001-9145068-002", "101-001-9145068-003", "101-001-9145068-004", "101-001-9145068-005"]) #demo 2
process_pending_trades(accounts[0], avg_spreads) #demo
