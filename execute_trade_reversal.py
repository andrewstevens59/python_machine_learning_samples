...
for row in setup_rows:
	...
	for order in orders:
		...
		if order.dir:
			buy_num += 1
			net_buy_amount += order.amount
			net_buy_position = (abs(order.open_price - curr_price) * order.amount) / pip_size
		else:
			sell_num += 1
			net_sell_amount += order.amount
			net_sell_position = (abs(order.open_price - curr_price) * order.amount) / pip_size

	min_profit = min(min_profit, total_order_pnl)
	

	if len(orders) > 0:
		trade_dir = net_buy_amount > net_sell_amount
		cursor = cnx.cursor()
		query = ("UPDATE trade_setups SET trade_dir = '" + str(int(trade_dir)) + "', min_profit = '" + str(min_profit) + "', stop_loss = '" + str(stop_loss_map[pair + "_" + api_key]) + "', last_update_time = '" + str(time.time()) + "', total_pnl = '" + str(total_order_pnl) + "', total_orders = '" + str(len(orders)) + "', total_margin_used = '" + str(total_order_margin) + "'  where api_key='" + api_key + "' and pair='" + pair + "'")
		cursor.execute(query)
		cnx.commit()
		cursor.close()
	...
	if len(orders) == 0:
		order_amount = total_balance * max_exposure * 0.01 * 100
		order_amount *= 6.0 / (barrier_in_pips * max_positions)
		...
		print pair, next_level
		if trade_dir:
			order_amount = abs(net_sell_position - net_buy_position) * 0.1
		else:
			order_amount = abs(net_buy_position - net_sell_position) * 0.1
