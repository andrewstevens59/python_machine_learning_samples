...
def close_group(group_metadata, balance, float_profit, margin_used, margin_available, orders):
	...
	if (time.time() - last_time > 12 * 60 * 60) or (margin_used / margin_available > 1.1):
		...
		if balance / equity_mean > 1.1 or float_profit / margin_used > 1.1 or (margin_used / margin_available > 1.1):
			...
			os.remove(root_dir + "group_metadata_basket")
			os.remove(root_dir + "order_metadata_basket")
			os.remove(root_dir + "model_wt")
			return

		group_metadata['equity_curve'].append(balance)
		group_metadata['group_processed_time'] = time.time()
		pickle.dump(group_metadata, open(root_dir + "group_metadata_basket", 'wb'))

def close_group_trades(pair, global_pnl, global_margin_used, orders_by_model, model_key, order_metadata, group_metadata, total_margin_available, total_margin_used, existing_order_amount, curr_price, avg_prices, avg_spreads, total_models, white_base_model_key_set, account_balance):
	...
	trade_logger.info('Model: ' + model_key + \
			...
			", Buy Margin: " + str(buy_margin_used) + \
			", Sell Margin: " + str(sell_margin_used) + \
			", Start Equity: " + str(group_metadata[model_key + "_start_equity"]) + \
	...
	for order in orders:
		...
		trade_logger.info(
			...
			", Dir: " + str(order.dir) + \
			", Predict: " + str('%0.2f' % order.curr_prediction) + \
			", Spread: " + str(group_metadata[order.prediction_key + "_curr_spread"]) + \
		...
		if abs(order.curr_prediction) >= 2 and ((order.curr_prediction > 0) != order.dir) and (order.base_amount >= 4 or account_balance > group_metadata[model_key + "_start_equity"]) and group_metadata[order.prediction_key + "_curr_spread"] < 5.0: 
		...
		if ((order.dir) != (buy_amount > sell_amount)) and (order.PnL > 0) and group_metadata[order.prediction_key + "_curr_spread"] < 5.0: 
			order_metadata, existing_order_amount, is_success = close_order(order, order_metadata, existing_order_amount, curr_price, commission, pip_size)
			if is_success == True:
				continue
...
def enter_group_trades(pair, white_model, white_base_model_key_set, order_metadata, group_metadata, orders_by_model, curr_model_wt, prev_model_wt, free_margin, used_margin, curr_price, account_numbers, existing_order_amount, curr_spread, avg_prices, account_balance):
	...
	base_model_key = model_key + "_" + str(white_model['entry_bias']) + "_" + str(white_model['trade_dir']) + "_" + str(martingale_type) + "_" + str(white_model['is_use_residual'])
	
	if base_model_key not in white_base_model_key_set:
		print "not in ", base_model_key
	...
	if base_model_key in orders_by_model:
		...
		for order in orders:
			...
			total_pnl += order.PnL
			max_amount = max(max_amount, order.base_amount)
		...
		group_metadata[base_model_key + "_start_equity"] = account_balance
	...
	if base_model_key + "_start_equity" not in group_metadata:
		group_metadata[base_model_key + "_start_equity"] = account_balance


	if abs(prediction) >= 2 and curr_spread < 1.5:

		trade_dir = prediction > 0
		if len(orders) > 0:
			if trade_dir == (buy_amount > sell_amount):
				return base_model_key

			if account_balance > group_metadata[base_model_key + "_start_equity"] :
				return base_model_key

			if trade_dir == True and max_buy_profit > 0:
				return base_model_key

			if trade_dir == False and max_sell_profit > 0:
				return base_model_key

		all_JPY = True
		for pair in white_model['currency_pairs']:
			if pair[4:7] != 'JPY':
				all_JPY = False
		...
		for pair in white_model['currency_pairs']:
			...
			new_order = Order()
			new_order.base_amount = max_amount * 2
			new_order.dir = trade_dir

			if all_JPY == True:
				order_amount = int(round((new_order.base_amount * curr_model_wt) / (100 * pair_mult)))
			else:
				order_amount = int(round((new_order.base_amount * curr_model_wt) / (pair_mult)))
...
def process_pending_trades(account_numbers, avg_spreads):
	...
	whitelist = pickle.load(open("/tmp/model_basket_predictions", 'rb'))

	if os.path.isfile(root_dir + "group_metadata_basket"):
		group_metadata = pickle.load(open(root_dir + "group_metadata_basket", 'rb'))
	else:
		group_metadata = {}

	if os.path.isfile(root_dir + "order_metadata_basket"):
		order_metadata = pickle.load(open(root_dir + "order_metadata_basket", 'rb'))
	else:
		order_metadata = {}

	if os.path.isfile(root_dir + "group_base_wt_basket"):
		prev_model_wt = pickle.load(open(root_dir + "group_base_wt_basket", 'rb'))
	else:
		prev_model_wt = {}
	...
	global_margin_used = 0
	global_pnl = 0

	for order in orders:

		global_margin_used += order.margin_used
		global_pnl += order.PnL
	...
	scalper_list = pickle.load(open(root_dir + "/trading_data/basket_model_whitelist", 'rb'))
	...
	for white_model in scalper_list:
		...
		curr_spread /= len(pairs)
		curr_price /= len(pairs)
	...
	for model in whitelist:
		...
		curr_model_wt = float(total_balance) * 0.05
		curr_spread = group_metadata[prediction_key + "_curr_spread"]

		base_model_key = enter_group_trades(pair, model, white_base_model_key_set, order_metadata, \
			group_metadata, orders_by_model, curr_model_wt, prev_model_wt, \
			total_margin_available, total_margin_used, curr_price, \
			account_numbers, existing_order_amount, curr_spread, avg_prices, total_balance + total_float_profit)

	for base_model_key in orders_by_model:
		close_group_trades(pair, global_pnl, global_margin_used, orders_by_model, base_model_key, \
		 	order_metadata, group_metadata, total_margin_available, total_margin_used, \
		  	...
		  	len(orders_by_model), white_base_model_key_set, total_balance + total_float_profit)

	pickle.dump(prev_model_wt, open(root_dir + "group_base_wt_basket", 'wb'))
	pickle.dump(group_metadata, open(root_dir + "group_metadata_basket", 'wb'))
	pickle.dump(order_metadata, open(root_dir + "order_metadata_basket", 'wb'))

	trade_logger.info('Global PnL: ' + str(global_pnl) + ", Global Margin: " + str(global_margin_used * 0.02)) 
...
trade_logger = setup_logger('first_logger', root_dir + "trade_basket.log")
...
if get_mac() == 154505288144005:
	...
	sftp.put("/tmp/pair_avg_spread", root_dir + "group_metadata_basket")
	sftp.put("/tmp/pair_avg_price", root_dir + "order_metadata_basket")
