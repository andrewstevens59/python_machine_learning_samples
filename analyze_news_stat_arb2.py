...
def back_test(currency1, currency2, price_df, pair, year, test_calendar, model_mean, description_map, feature_importances, interest_times):
	...
	for index, row in test_calendar.iterrows():
		...
		mean_prediction = model_mean.predict(tf.convert_to_tensor([global_feature_vector]), steps=1)[0][0]
...
interest_rate_df = pickle.load(open("/Users/callummc/interest_data.pickle", 'rb'))

def align_interest_rates(pair, interest_rate_df, price_df):

	print "align reates"

	test_interest_df = interest_rate_df[(interest_rate_df["CURRENCY"] == pair[0:3])]
	test_interest_df.sort_values(by=['DATE'], inplace=True)
	interest_rates1 = test_interest_df.T.to_dict().values()

	test_interest_df = interest_rate_df[(interest_rate_df["CURRENCY"] == pair[4:7])]
	test_interest_df.sort_values(by=['DATE'], inplace=True)
	interest_rates2 = test_interest_df.T.to_dict().values()


	current_index_curr1 = 0
	current_index_curr2 = 0
	...
	for index, row in price_df.iterrows():
		time_stamp = row['times']

		while current_index_curr1 + 1 < len(interest_rates1) and time_stamp >= interest_rates1[current_index_curr1 + 1]["DATE"]:
			current_index_curr1 += 1

		while current_index_curr2 + 1 < len(interest_rates2) and time_stamp >= interest_rates2[current_index_curr2 + 1]["DATE"]:
			current_index_curr2 += 1

		bids_currency1.append(interest_rates1[current_index_curr1]["BID"])
		asks_currency1.append(interest_rates1[current_index_curr1]["ASK"])

		bids_currency2.append(interest_rates2[current_index_curr2]["BID"])
		asks_currency2.append(interest_rates2[current_index_curr2]["ASK"])

	price_df["BID_int_curr1"] = bids_currency1
	price_df["BID_int_curr2"] = bids_currency2

	price_df["ASK_int_curr1"] = asks_currency1
	price_df["ASK_int_curr2"] = asks_currency2
...
for year in range(2013, 2019):
	...
	total_pnl = 0
	for currency_pair in ["EUR_GBP", "AUD_CAD", "NZD_CAD", "AUD_NZD"]:#["EUR_JPY", "USD_JPY", "GBP_JPY", "NZD_JPY", "CAD_JPY", "CHF_JPY", "AUD_JPY"]:#["EUR_JPY", "USD_JPY"]:#, "EUR_AUD", "EUR_CAD", "EUR_GBP", "AUD_CAD", "GBP_CAD", "NZD_CAD", "AUD_NZD"]:
	...
	#["EUR_JPY", "USD_JPY", "EUR_AUD", "EUR_CAD", "EUR_GBP", "AUD_CAD", "GBP_CAD", "NZD_CAD", "AUD_NZD"]:
		...
		for test_pair in [currency_pair]:
			...
			model = keras.Sequential([
			    layers.Dense(64, activation=tf.nn.relu, input_shape=[len(X_train[0])]),
			...
			tensor_from_list = tf.convert_to_tensor(X_train)
			print tensor_from_list

			#clf_mean = GradientBoostingRegressor(random_state=42)
			model.fit(tensor_from_list, y_train, steps_per_epoch=len(X_train))
			...
			equity_curve, returns, avg_amount = back_test(first_currency, second_currency, price_df, test_pair, year, test_calendar, model, description_map, feature_importances, interest_times)
			if len(equity_curve) == 0:
				continue

			if True or test_pair != "EUR_GBP":
