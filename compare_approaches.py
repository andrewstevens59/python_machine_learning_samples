...
def get_weight(predict, weight, is_reverse):
	if len(weight) == 1:
		weight = weight[0]
	else:
		if len(weight[0]) == 0 or len(weight[1]) == 0:
			weight = 0
		else:
			if (predict > 0) == is_reverse:
				if len(weight[1]) > 0:
					weight = (weight[1][0])
				else:
					weight = 0
...
def classification_model_sharpe(X, y, weights):
	...
	threshold = upper_percentile

	start = 0
	cross_fold_num = 5
	end = len(X) / cross_fold_num
	...
	for i in range(cross_fold_num):
		...
		for predict, actual, w in zip(trend_preds, y[start:end], weights[start:end]):

			if abs(predict - 0.5) > threshold:# and ((predict - 0.5) > 0) == w:
		...
		start += len(X) / cross_fold_num
		end += len(X) / cross_fold_num

	return returns1


def regression_model_sharpe(X, y, weights):
	...
	upper_percentile = percentile_thresholds[90]
	threshold = upper_percentile
	...
	for i in range(cross_fold_num):
		...
		for predict, actual, w in zip(trend_preds, y[start:end], weights[start:end]):
			...
			if abs(predict) > threshold:# and ((predict) > 0) == w:
				if (predict > 0) == (actual > 0):
					returns1.append(1)
				else:
					returns1.append(-1)
...
def store_model_comparisons():

	for currency_pair in currency_pairs:

		if os.path.isfile("models/comparisons_{}.pickle".format(currency_pair)) == False:
			continue

		measures = pickle.load(open("models/comparisons_{}.pickle".format(currency_pair), "rb"))

		for key in measures:
			if len(measures[key]) > 2:
				sharpe = np.mean(measures[key]) / np.std(measures[key])
				if sharpe > 0:
					print (currency_pair, key, sharpe, len(measures[key]))

#store_model_comparisons()
#sys.exit(0)

def evaluate_barrier_model_measures(all_returns, currency_pair, pip_size, price_df, barrier_models, barrier_model_scores, X_train_map, y_train_map, target_barrier = None):


	descriptions = list(calendar_df["description"].unique())

	measures = {"barrier" : [], "regression" : [],  "binary" : []}

	for description in descriptions:

		for key in measures:
			if len(measures[key]) > 2:
				print (currency_pair, key, np.mean(measures[key]) / np.std(measures[key]))


		print ("save")
		pickle.dump(measures, open("models/comparisons_{}.pickle".format(currency_pair), "wb"))
		...
		for currency in ["AUD", "GBP", "CAD", "NZD", "USD", "EUR", "JPY", "CHF"]:
			...
			for hour_after_it in range(2):
				...
				barrier_indexes = [rand.randint(1, 50) for barrier_it in range(2)]
				...
				for approach in ["barrier", "regression"]:
					...
					for index, row in sub_df.iterrows():
						...
						past_price_df = price_df[price_df['times'] <= curr_time]
						if len(past_price_df) == 0:
							continue

						past_prices = (past_price_df)['prices_target'].values.tolist()

						future_prices = (future_price_df)['prices_target'].values.tolist()

						before_price_df = price_df[price_df['times'] < curr_time]
						before_prices = (before_price_df)['prices_target'].values.tolist()

						price_diffs = [past_prices[-1] - past_prices[-min(len(past_prices)-1, time_range)] for time_range in [5 * 24, 10 * 24, 15 * 24, 20 * 24]]
						pos_price_diffs = [abs(v) for v in price_diffs if v > 0]
						neg_price_diffs = [abs(v) for v in price_diffs if v < 0]

		
						weight = [pos_price_diffs, neg_price_diffs]
						...
						before_prices = before_prices[-24 * 5:]
						before_prices = [[index, p] for p in enumerate(before_prices)]
						pos_before_prices = [p[0] for p in before_prices if p[1] > 0]
						neg_before_prices = [p[0] for p in before_prices if p[1] < 0]
						weight = (np.mean(pos_before_prices) > np.mean(neg_before_prices))
							...
							if approach == "barrier":
								...
								for price in future_prices:
									...
									if price >= top_barrier:
										...
										price_delta[barrier_index].append(weight)
										break
					...
					for barrier_index in y_train_map:
						...
						weights = price_delta[barrier_index]
		
						if approach == "barrier" or approach == "binary":
							returns1 = classification_model_sharpe(X, y, weights)
						else:
							returns1  = regression_model_sharpe(X, y, weights)

						measures[approach] += returns1
...
def back_test_news_calendar(model_type):
	...
	all_returns = []
	for currency_pair in [sys.argv[2]]:
		...
		evaluate_barrier_model_measures(all_returns, currency_pair, pip_size, price_df, {}, {}, {}, {})
